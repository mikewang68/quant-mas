...
def test_ip_rotation_integration():
    ...
    # 模拟一个需要IP切换的网络错误
    test_error = Exception("Connection aborted. RemoteDisconnected('Remote end closed connection without response')")

    print("   模拟网络连接错误...")
    print("   注意: 这将实际执行IP轮换机制")

    # 询问用户是否继续
    response = input("   是否继续执行IP轮换测试？(y/N): ")
    if response.lower() != 'y':
        print("   跳过实际IP轮换测试")
        return

    print("\n   开始处理网络错误...")
    success = handle_network_error(test_error)

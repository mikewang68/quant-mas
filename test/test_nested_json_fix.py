#!/usr/bin/env python3
"""
Test the nested JSON fix in the LLM fundamental strategy
"""
...
def test_nested_json_fix():
    """Test the nested JSON handling fix"""

    # Create strategy instance
    strategy = LLMFundamentalStrategy(name="Test LLM Strategy", params={})

    # Test case 1: Normal JSON response
    print("Test 1: Normal JSON response")
    normal_response = '''{
        "score": 0.65,
        "value": "This is the analysis text"
    }'''

    try:
        result = strategy.get_llm_analysis(normal_response)
    ...
    # Test case 2: Nested JSON response (the problematic case)
    print("Test 2: Nested JSON response")
    nested_response = '''{
        "score": 0.65,
    ...
    try:
        result = strategy.get_llm_analysis(nested_response)
        print(f"Score: {result.get('score')}")
        print(f"Value: {result.get('value')}")
    ...
    # Test case 3: Actual LLM response format
    print("Test 3: Actual LLM response format")
    actual_response = '''{
    "score": 0.65,
    ...
    try:
        result = strategy.get_llm_analysis(actual_response)
        print(f"Score: {result.get('score')}")
        print(f"Value type: {type(result.get('value'))}")
        value_str = str(result.get('value', ''))[:200]
        print(f"Value (first 200 chars): {value_str}")

        # Check if score is in value

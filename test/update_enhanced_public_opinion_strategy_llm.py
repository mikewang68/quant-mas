#!/usr/bin/env python3
"""
Script to update the enhanced public opinion analysis strategy LLM configuration in the MongoDB database
"""
...
def update_enhanced_public_opinion_strategy_llm():
    """Update the enhanced public opinion analysis strategy LLM configuration in the database"""
    try:
        # Initialize MongoDB manager
        db_manager = MongoDBManager()

        # Define the updated enhanced public opinion analysis strategy with simplified LLM config
        updated_strategy = {
            "name": "增强型舆情分析策略",
            "type": "public_opinion",
        ...
        if result.upserted_id or result.modified_count > 0:
            print(f"Successfully updated enhanced public opinion strategy LLM configuration")
            ...
            print(f"  LLM name: {updated_strategy['parameters']['llm_name']}")
        else:
        ...
        print(f"Error updating enhanced public opinion strategy LLM configuration: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    update_enhanced_public_opinion_strategy_llm()

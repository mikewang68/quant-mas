...
def debug_actual_response():
    ...
    # This is the actual response we're seeing in the database
    actual_response = '''{
    "score": 0.65,
    ...
    try:
        ...
        if "0.65" in value_str:
            print("*** ISSUE: Score still found in value field ***")
        else:
            print("+++ FIXED: Score not found in value field +++")

        # Check if the value contains the phrase about scoring
        if "基本面评分是0.65" in value_str:
            print("*** ISSUE: Scoring phrase found in value field ***")
        else:
            print("+++ FIXED: Scoring phrase not found in value field +++")

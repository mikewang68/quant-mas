#!/usr/bin/env python3
"""
Debug the regex extraction logic
"""
...
def debug_regex_extraction():
    ...
    print("Debugging regex extraction...")
    print(f"Response length: {len(problematic_response)}")

    # Test score extraction
    score_matches = re.findall(r'(?:评分|score)[:：]?\s*(\d+\.?\d*)', problematic_response, re.IGNORECASE)
    ...
    if score_matches:
        try:
            ...
            print(f"Failed to parse extracted score: {e}")
    ...
    for i, pattern in enumerate(value_patterns):
        ...
        match = re.search(pattern, problematic_response, re.DOTALL | re.IGNORECASE)
        if match:
            extracted_value = match.group(1).strip()
            print(f"Match found, extracted value length: {len(extracted_value)}")
            print(f"Extracted value (first 200 chars): {extracted_value[:200]}")
        else:

...
# -*- coding: utf-8 -*-
"""
Script to analyze the actual pool data structure and count strategies
"""
...
def analyze_pool_structure():
    ...
    try:
        ...
        if pool_stocks:
            ...
            # Count strategies by examining fields (excluding 'code' and 'signal')
            strategy_count = 0
            strategy_fields = []

            for field_name, field_value in first_stock.items():
                # Skip non-dict fields and the special fields
                if field_name in ['code', 'signal'] or not isinstance(field_value, dict):
                    if field_name in ['code', 'signal']:
                        print(f"  Skipping field '{field_name}' (special field)")
                    else:
                        print(f"  Skipping field '{field_name}' (not a dict)")
                    continue
...
def analyze_002067_in_pool():
    ...
    try:
        ...
        # Count total strategies in pool (from first stock)
        first_stock = pool_stocks[0]
        total_strategies = 0
        for field_name, field_value in first_stock.items():
            if field_name in ['code', 'signal'] or not isinstance(field_value, dict):
        ...
        print(f"Total strategies in pool: {total_strategies}")

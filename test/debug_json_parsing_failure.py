#!/usr/bin/env python3
"""
Debug what happens when JSON parsing fails
"""
...
def debug_json_parsing_failure():
    """Debug what happens when JSON parsing fails"""

    # Create strategy instance
    strategy = LLMFundamentalStrategy(name="Test LLM Strategy", params={})

    # This is the exact response that causes JSON parsing to fail
    ...
    print("Testing JSON parsing failure scenario...")

    # Try to parse the JSON directly
    try:
        result = json.loads(problematic_response)
        print("JSON parsing succeeded - this is unexpected!")
        print(f"Result: {result}")
    except json.JSONDecodeError as e:
        print(f"JSON parsing failed as expected: {e}")
        print(f"Error position: {e.pos}")

        # When JSON parsing fails, the get_llm_analysis method should return raw content
        print("\nSimulating get_llm_analysis fallback behavior:")
        fallback_result = {
            "score": 50.0,  # Default neutral score
            "value": problematic_response,
        }
        print(f"Fallback result score: {fallback_result['score']}")
        print(f"Fallback result value length: {len(fallback_result['value'])}")

        # Check if score is in value (this is the issue we're seeing)
        if "0.65" in fallback_result['value']:
            print("*** ISSUE CONFIRMED: Score found in fallback value ***")

if __name__ == "__main__":
    debug_json_parsing_failure()

"""
Hull Moving Average Price Acceleration with Turnover Filter Strategy
A strategy that selects stocks based on price acceleration confirmed by turnover rate using HMA.\n"""
...
class HMATurnoverStrategy(BaseStrategy):
    """
Hull Moving Average Price Acceleration with Turnover Filter Strategy
Selects stocks based on price acceleration confirmed by turnover rate using HMA.\n    """

    def __init__(self, name: str = "船体均线价格加速换手率过滤策略", params: Optional[Dict] = None):
        """
        Initialize the Hull Moving Average Price Acceleration with Turnover Filter strategy

        Args:
            name: Strategy name
            params: Strategy parameters
                - short_period: Short HMA period (default: 5)
                - mid_period: Mid HMA period (default: 13)
    ...
    def calculate_hma(self, data: pd.DataFrame, period: int) -> np.ndarray:
        """
        Calculate HMA (Hull Moving Average) for given period
        ...
        close_prices = data['close'].values.astype(np.double)  # Convert to numpy array for TA-Lib

        # Calculate the three WMA components
        half_period = int(period / 2)
        sqrt_period = int(np.sqrt(period))

        # First WMA with half period
        wma_half = talib.WMA(close_prices.astype(np.double), timeperiod=half_period)

        # Second WMA with full period
        wma_full = talib.WMA(close_prices.astype(np.double), timeperiod=period)

        # Calculate 2 * WMA(n/2) - WMA(n)
        diff_wma = 2 * wma_half - wma_full

        # Final WMA with sqrt period
        hma_values = talib.WMA(diff_wma.astype(np.double), timeperiod=sqrt_period)
    ...
    def analyze(self, data: pd.DataFrame) -> tuple[bool, str, Optional[float]]:
        ...
        try:
            ...
            hma_short = self.calculate_hma(data, self.short_period)
            hma_mid = self.calculate_hma(data, self.mid_period)

            # Get current values
            hma_short_current = hma_short[-1] if not np.isnan(hma_short[-1]) else None
            hma_mid_current = hma_mid[-1] if not np.isnan(hma_mid[-1]) else None

            # Get previous short HMA value
            hma_short_previous = hma_short[-2] if len(hma_short) > 1 and not np.isnan(hma_short[-2]) else None

            # Check if all values are valid
            if hma_short_current is None or hma_mid_current is None or hma_short_previous is None:
                return False, "HMA计算无效", None

            # Check conditions:
            # 1. short > mid
            # 2. short >= previous short
            condition_1 = hma_short_current > hma_mid_current
            condition_2 = hma_short_current >= hma_short_previous

            if not condition_1 or not condition_2:
                reason = "不满足条件: "
                if not condition_1:
                    reason += f"HMA{self.short_period}({hma_short_current:.2f}) <= HMA{self.mid_period}({hma_mid_current:.2f})"
                if not condition_2:
                    if not condition_1:
                        reason += "; "
                    reason += f"HMA{self.short_period}当前值({hma_short_current:.2f}) < 前值({hma_short_previous:.2f})"
                return False, reason, None

            # Calculate score based on strength of conditions
            # Score components:
            # 1. HMA spread (40% weight)
            # 2. HMA acceleration (30% weight)
            # 3. Turnover rate within optimal range (30% weight)
            hma_spread_score = min(1.0, max(0.0, (hma_short_current - hma_mid_current) / hma_mid_current * 100))
            hma_acceleration_score = min(1.0, max(0.0, (hma_short_current - hma_short_previous) / hma_short_previous * 100))
            ...
            score = (hma_spread_score * 40 + hma_acceleration_score * 30 + turnover_score * 30)

            reason = (f"满足条件: HMA{self.short_period}({hma_short_current:.2f}) > HMA{self.mid_period}({hma_mid_current:.2f}), "
                     f"HMA{self.short_period}当前值({hma_short_current:.2f}) >= 前值({hma_short_previous:.2f}), "
            ...
            try:
                ...
                if meets_criteria:
                    ...
                    if not data.empty and len(data) >= max(self.short_period, self.mid_period):
                        # Calculate HMA values for technical analysis
                        hma_short_val = self.calculate_hma(data, self.short_period)[-1]
                        hma_mid_val = self.calculate_hma(data, self.mid_period)[-1]

                        technical_analysis = {
                            'price': float(data['close'].iloc[-1]),
                            'hma_short': float(hma_short_val),
                            'hma_mid': float(hma_mid_val),
                            'turnover_rate': float(data['turnover_rate'].iloc[-1]) if 'turnover_rate' in data.columns else 0.0
                        }

                    # Calculate position based on score
                    position = 0.0 # calculate_position_from_score(score)  # Assuming a function exists
                    ...
                    selected_stocks.append({
                        ...
                        'uptrend_accelerating': False,  # Add uptrend_accelerating field
                        'value': reason
                    })

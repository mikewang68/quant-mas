"""
Scalping Strategy
A high-frequency strategy that generates buy/sell signals based on short-term price movements.
"""
...
class ScalpingStrategy(BaseStrategy):
    """
    Scalping Strategy
    Generates buy signals on short-term price increases,
    and sell signals on short-term price decreases.
    """
    
    def __init__(self, name: str = "Scalping", params: Optional[Dict] = None):
        """
        ...
        Args:
            ...
            params: Strategy parameters
                - period: Lookback period for momentum calculation (default: 5)
                - threshold: Price change threshold as percentage (default: 0.005)
        ...
        self.threshold = self.params.get('threshold', 0.005)  # 0.5% threshold
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        price_change_ratio = momentum / close_prices
        ...
        signals['price_change_ratio'] = price_change_ratio
        signals['signal'] = 'HOLD'
        signals['position'] = 0.0
        
        # Generate signals
        for i in range(1, len(signals)):
            # Buy signal: Positive momentum exceeds threshold
            if (price_change_ratio[i] >= self.threshold and 
                price_change_ratio[i-1] < self.threshold):
                signals.loc[i, 'signal'] = 'BUY'
                signals.loc[i, 'position'] = 1.0
            
            # Sell signal: Negative momentum exceeds threshold
            elif (price_change_ratio[i] <= -self.threshold and 
                  price_change_ratio[i-1] > -self.threshold):

...
class ThreeMAGoldenCrossStrategy(BaseStrategy):
    ...
    def __init__(
        self,
        name: str = "Three MA Golden Cross Strategy",
    ...
    def analyze(self, data: pd.DataFrame) -> Tuple[bool, str, Optional[float], bool]:
        ...
        Returns:
            Tuple of (meets_criteria, selection_reason, score, golden_cross)
        """
        if data.empty or len(data) < max(
            self.params["ma_short"], self.params["ma_mid"], self.params["ma_long"]
        ):
    ...
    def _detect_golden_cross(self, data: pd.DataFrame) -> bool:
        ...
        if (
            data.empty
            or len(data) < max(self.params["ma_short"], self.params["ma_mid"]) + 1
        ):
            return False
                        ...
                        if len(close_prices) >= max(
                            ...
                            technical_analysis = {
                                ...
                                "moving_averages": {
                                    "ma_short": float(
                                        np.mean(
                                            close_prices[-self.params["ma_short"] :]
                                        )
                                    ),
                                    "ma_mid": float(
                                        np.mean(close_prices[-self.params["ma_mid"] :])
                                    ),
                                    "ma_long": float(
                                        np.mean(close_prices[-self.params["ma_long"] :])
                                    ),
                                },
                            }

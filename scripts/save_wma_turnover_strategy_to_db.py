#!/usr/bin/env python3
"""
Save WMA Turnover Strategy to MongoDB database
"""
...
def save_wma_turnover_strategy():
    """Save WMA Turnover Strategy to database"""
    ...
    try:
        ...
        strategy_data = {
            "name": "均线价格加速换手率过滤策略",
            "type": "technical",
            "description": "利用短期加权移动平均线(WMA)的价格加速特性，结合换手率过滤来识别具有上涨动力且流动性适中的股票。当短期WMA高于中期WMA且短期WMA呈上升趋势时，如果换手率在合理范围内(5-25%)，则产生买入信号。",
            "parameters": {
                "short_period": 5,
                "mid_period": 13,
                "min_turnover": 5.0,
            ...
            "program": {
                "file": "wma_turnover_strategy.py",
                "class": "WMATurnoverStrategy"
            }
        }
...
if __name__ == "__main__":
    success = save_wma_turnover_strategy()
    if success:
        print("WMA Turnover Strategy saved to database successfully")
        sys.exit(0)
    else:
        print("Failed to save WMA Turnover Strategy to database")
        sys.exit(1)
